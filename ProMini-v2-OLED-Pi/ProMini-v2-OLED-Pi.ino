...
unsigned long odometr, odometr_old, trip;
...
void setup(){
    ...
    attachInterrupt(0, spd, FALLING);                                   // Speedometer falling interrupt
    attachInterrupt(1, tah, RISING);                                    // Tahometer rising interrupt

    mcp.begin(7);                                                       // addr 7 = A2 high, A1 high, A0 high 111
    for (pin = 0; pin  < 16; pin ++) { mcp.pinMode(pin, INPUT); }       // MCP23017 readonly init


    odometr = EEPROM_ulong_read(0);
    trip = EEPROM_ulong_read(1);
    odometr_old = odometr;
...
void loop(){
    ...
    if(((millis() - millis_t) >= 20) && digitalRead(ACC)){              // Serial refresh interval in milliseconds if ACC present
        ...
        Serial.print(speedometer + "," + tahometer + "," + voltage + "," + fuel + "," + temperature + "," + dallastemp + mcparray + "," + odometr +":\n");
    ...
    if(((millis() - millis_d) >= 100) && digitalRead(ACC)){             // OLED refresh interval in milliseconds if ACC present
        ...
        u8x8.setCursor(8, 11); u8x8.print(odometr);
        u8x8.setCursor(8, 12); u8x8.print(trip);
    ...
    if (!digitalRead(ACC) && odometr > odometr_old) {                   // Write EEPROM if not ACC present
        //EEPROM_ulong_write(0, odometr);
        //EEPROM_ulong_write(1, trip);
        delay(2000);
    }
}
